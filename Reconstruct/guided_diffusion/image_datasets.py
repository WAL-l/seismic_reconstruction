...
class ImageDatasetInpa(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pil_gt = np.load(gt_path)

        mask_path = self.local_masks[idx]
        pil_mask = np.load(mask_path)
        ...
        np.save(f"./log/{os.path.basename(gt_path)}_gt_noguiyi", arr_gt)

        np.save(f"./log/{os.path.basename(gt_path)}_mask_noguiyi", arr_mask)
...
def center_crop_arr(pil_image, image_size):
    ...
    while min(*pil_image.shape) >= 2 * image_size:
        pil_image=np.resize(pil_image,
            tuple(x // 2 for x in pil_image.shape)
        )

    scale = image_size / min(*pil_image.shape)
    pil_image=np.resize(pil_image,
        tuple(round(x * scale) for x in pil_image.shape)
    )
